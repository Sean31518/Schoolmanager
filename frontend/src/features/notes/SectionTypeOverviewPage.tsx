...
export function SectionTypeOverviewPage() {
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-slate-800">Klassenstufen</h1>

      {isLoading ? (
        <p className="mt-4 text-slate-400">Lädt...</p>
          ...
          {existingGrades.map((grade) => (
            <li key={grade}>
              <Link
                ...
                className="block rounded-lg bg-white px-4 py-2 shadow-sm hover:shadow"
              >
                Klasse {grade}
              </Link>
            </li>
        ...
        <p className="mt-4 text-slate-400">Noch keine Klassenstufe angelegt.</p>
      )}
